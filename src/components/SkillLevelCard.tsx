...
import { ChevronDown, ChevronUp, Clock, Target, ExternalLink } from 'lucide-react';
...
const SkillLevelCard = ({ level, title, eloRange, description, topics, colorScheme }: SkillLevelCardProps) => {
  ...
  return (
    <Card className={cn(
      ...
      <div className="text-center mb-6">
        <Badge variant="secondary" className={cn("mb-3", `bg-${level}/20 text-${level}`)}>
          {eloRange}
        </Badge>
        <h3 className={cn("text-2xl font-bold mb-2", `text-${level}`)}>{title}</h3>
          ...
          >
            ...
            >
              <div className="flex-1">
                ...
                <p className="text-sm text-foreground/60 mt-1">{topic.description}</p>
            ...
            {expandedTopics.has(topic.id) && (
              <div className="mt-4 pt-4 border-t border-white/10 animate-fade-in">
                {/* Skills */}
                <div className="mb-4">
                  <h5 className="text-sm font-medium text-foreground/80 mb-2">Skills You'll Learn:</h5>
                ...
                {topic.prerequisites.length > 0 && (
                  <div className="mb-4">
                    <h5 className="text-sm font-medium text-foreground/80 mb-2">Prerequisites:</h5>
                ...
                {/* Resources */}
                <div>
                  <h5 className="text-sm font-medium text-foreground/80 mb-3">Recommended Resources:</h5>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <h6 className="font-medium text-foreground text-sm">{resource.title}</h6>
                            ...
                            <p className="text-xs text-foreground/60 mb-2">{resource.description}</p>
                          </div>
                          <Button
                            size="sm"
                            variant="outline"
